...
public:
	...
	void calculateMovement() {
		...
		vy = 0;
		vz = 0;
		vec3 vel=sub_vec3(destination, { x,y,z });
		vel=vel.normalize();
	...
	void calculatePredictedMovement() {
		...
		if (pftTimer > 200) {
			if (distance(x, y, z, pftStartPosition.x, pftStartPosition.y, pftStartPosition.z) < r * 2) {
				dir = getRandomDir();
			}
			pftTimer = 0;
		}
...
void write_frame_to_file() {
	FILE* fp_out;
	std::string path = simulationName+"/"+ std::string(FRAMES_OUT_PATH) +std::string("frame") + std::to_string(frame) + std::string(".txt");
...
int main(int argc, char** argv) {
	...
	int i;
	std::cout << "enter simulation. 0-verticle bridge,1-horizantal bridge: ";
	std::cin >> i;
	printf("int %i", i);
	switch (i) {
	case 0: {
		...
		//freezing ants
		for (j; j < 150; j++) {
			vec3 pos = spawnRandomCircle({ 0,0,0 }, 2, 0);
			particles.push_back(Particle(pos.x, pos.y, pos.z, j));
			particles[j].setDestination({ 0, 1.5,0.1 });
			particles[j].setSleep(1);
		}
		for (j; j < 200; j++) {
			vec3 pos = spawnRandomCircle({ 0,1.5,0 }, 2, 0);
			particles.push_back(Particle(pos.x, pos.y, pos.z, j));
			particles[j].setDestination({ 0, 0,0.1 });
			particles[j].setSleep(1);
		}
	...
	case 1: {
		...
		for (j; j < 150; j++) {
			vec3 pos = spawnRandomCircle({ -1,1,0 }, 2, 2);
			particles.push_back(Particle(pos.x, pos.y, pos.z, j));
			particles[j].setDestination({ 3, 2,0 });
			particles[j].setSleep(1);
		}
