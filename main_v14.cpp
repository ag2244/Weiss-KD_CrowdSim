...
#define SETUP_OUT_PATH "setup/"
#define FRAMES_OUT_PATH "frames/"
...
void display() {
	...
	if (debug==true) {
		...
		drawSphere(particles[0].closestPoint.x, particles[0].closestPoint.y, particles[0].closestPoint.z, particles[0].r);
		drawSphere(particles[50].destination.x, particles[50].destination.y, particles[50].destination.z, particles[50].r);
		drawSphere(particles[50].closestPoint.x, particles[50].closestPoint.y, particles[50].closestPoint.z, particles[50].r);
	...
	for (int i = 0; i < particles.size(); i++) {
		...
		if (debug == true) {
			if (particles[i].isStatic) {
				...
				//glEnable(GL_LIGHTING);
				//drawLine(cameraPos.x, cameraPos.y, cameraPos.z, particles[0].x, particles[0].y, particles[0].z);
			}
			else {
				if (i < 50)
			...
			//drawLine(particles[i].x, particles[i].y, particles[i].z, particles[i].x + particles[i].up.x * 2, particles[i].y + particles[i].up.y * 2, particles[i].z + particles[i].up.z * 2);
...
bool LineSphereintersect(vec3 start,vec3 end, Particle s)
{
	 vec3 LineDiffVect = sub_vec3(end,start);
	 float lineSegSqrLength = length_vec3(LineDiffVect);

	 vec3 LineToPointVect = sub_vec3({s.x,s.y,s.z},start);
	 float dotProduct = dot_vec3(LineDiffVect,LineToPointVect);
	...
	else if (percAlongLine > 1.0f)
	{
	 ...
	 vec3 IntersectionPt = (start + (percAlongLine * (end - start)));

	 vec3 SpherePtToIntersect = IntersectionPt - vec3(s.x,s.y,s.z);
	 float SqrLengSphereToLine = length_vec3(SpherePtToIntersect);

	return (SqrLengSphereToLine <= s.r);
}
template <typename T>
bool LineSphereIntersection(T sphere, vec3 v0, vec3 v1) {
	...
	//printf("a:%f", a);
	//printf("v0:%f,%f,%f v1:%f,%f,%f ", v0.x, v0.y, v0.z, v1.x, v1.y, v1.z);
...
std::pair<int, char>  getClosestObsticle(vec3 pos, int id) {
	...
	if (isgreaterequal(staticAnt.second, platform.second)) {
...
void clamp_particle_to_point(int i, vec3 point) {
	...
	vec3 closestToSphere = getClosestPointToSphere({ particles[i].px,particles[i].py,particles[i].pz }, particles[i].r , point);
...
vec3 get_cube_normal_from_position(T cube, vec3 pos) {
	vec3 closestPoint = closestPointOnAABB(pos, cube);
	return sub_vec3(pos, closestPoint).normalize();
}


vec3 get_platform_normal_from_position(int id, vec3 pos) {
	Platform p = getPlatform<Platform>(id);
	vec3 closestPoint;
	if (p.tp == box)
		closestPoint= closestPointOnAABB(pos, p);
	else if (p.tp == sphere)
		closestPoint= closestPointOnSphere(pos, p);
	return sub_vec3(pos, closestPoint).normalize();
}

vec3 get_ant_normal_from_position(int id, vec3 pos) {
	vec3 closestPoint = closestPointOnSphere(pos, particles[id]);
	return sub_vec3(pos, closestPoint).normalize();
}
...
float* axis_aligned_cube_collision_constraint(Particle p1, Platform cube) {
	...
	vec3 norm = get_cube_normal_from_position(cube, { p1.px,p1.py,p1.pz });
...
void write_frame_to_file() {
	FILE* fp_out;
	std::string path = simulationName + "/" + std::string(FRAMES_OUT_PATH) + std::string("frames") + std::string(".txt");
	fp_out = fopen(path.c_str(), "a");
	if (fp_out != NULL) {
		for (int i = 0; i < num_of_particles; i++) {
			if (i != num_of_particles - 1)
				fprintf(fp_out, "%.5f,%.5f,%.5f,", particles[i].x, particles[i].y, particles[i].z);
			else
				fprintf(fp_out, "%.5f,%.5f,%.5f\n", particles[i].x, particles[i].y, particles[i].z);
		}
	}
	fclose(fp_out);
}
...
void createSetUpFile(int sim) {
	FILE* fp_out;
	std::string path = simulationName + std::string("/setup") + std::string(".txt");
	...
	if (fp_out != NULL) {
		...
		for (int i = 0; i < platforms.size(); i++) {
			if (platforms[i].tp == sphere)
				...
				fprintf(fp_out, "box,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f\n", platforms[i].w, platforms[i].l, platforms[i].h, platforms[i].x, platforms[i].y, platforms[i].z);
		}
	}
	fclose(fp_out);
}
...
void handle_traffic() {
	for (int i = 0; i < particles.size()-1; i++) {
		if (particles[i].isStatic)
			continue;
		for (int j = i+1; j < particles.size(); j++) {
			if (particles[j].isStatic)
				continue;
			//bool coll = false;
			//printf("coll %i,%i", i, j);
			bool coll = LineSphereintersect({ particles[i].x,particles[i].y,particles[i].z }, { particles[i].x + particles[i].forward.x * .5f, particles[i].y + particles[i].forward.y * .5f, particles[i].z + particles[i].forward.z * .5f }, particles[j]);
				//LineSphereIntersection(particles[j], { particles[i].px,particles[i].py,particles[i].pz }, { particles[i].vx,particles[i].vy,particles[i].vz });
			//printf("coll %i", coll);
			if (coll==true) {
				//printf("potential collision");
				//particles[i].steerVelocity();
				vec3 moveForce = scale_vec3(particles[i].right, particles[i].r);
				vec3 moveForce2 = scale_vec3(particles[j].right, particles[j].r);
				particles[i].px += moveForce.x * .99 * time_delta;
				particles[i].py += moveForce.y * .99 * time_delta;
				particles[i].pz += moveForce.z * .99 * time_delta;
				particles[j].px += moveForce2.x * .99 * time_delta;
				particles[j].py += moveForce2.y * .99 * time_delta;
				particles[j].pz += moveForce2.z * .99 * time_delta;
				//particles[i].state = Static;
...
void pbd_main_loop(int a)
{
	...
	for (int i = 0; i < particles.size(); i++) {
		...
		particles[i].px = particles[i].x + particles[i].pforwardx;
		if (particles[i].attachedType == 'p')
			particles[i].py = particles[i].y + particles[i].pforwardy;
		else
			particles[i].py = particles[i].y + particles[i].pforwardy+gravity * time_delta;
		particles[i].pz = particles[i].z + particles[i].pforwardz;
	}
...
vec3 spawnRandomCircle(vec3 center, float radius, int dir, float ang) {
...
int main(int argc, char** argv) {
	...
	case 0: {
		...
		for (j; j < 50; j++) {
			float ang = 1 * (j + 1);
			vec3 pos = spawnRandomCircle({ 0,0,0 }, 1, 0, ang);
			particles.push_back(Particle(pos.x, pos.y, pos.z, j));
			int rem = j % 10;
			if (rem != 0) {
				particles[j].follow = true;
				particles[j].followId = j - 1;
				particles[j].speed = .025;
				particles[j].state = FollowLeader;
				//particles[j].setDestination({ 0, 1.5,0.1 });
			}
			else {
				particles[j].setDestination({ 0, 2,0.1 });
				particles[j].maxTimer = 200;
				particles[j].state = FollowDestination;
				//particles[j].scout = true;
				//particles[j].mass = 500;
		...
		for (j; j < 100; j++) {
			float ang = 1 * (j + 1);
			vec3 pos = spawnRandomCircle({ 0,1.5,0 }, 1, 0, ang);
			particles.push_back(Particle(pos.x, pos.y, pos.z, j));
			//particles[j].setDestination({ 0, 0,0.1 });
			int rem = j % 10;
			if (rem != 0) {
				particles[j].follow = true;
				particles[j].followId = j - 1;
		...
		//for (j; j < 150; j++) {
		//	float ang = 1 * (j + 1);
		//	vec3 pos = spawnRandomCircle({ 0,0,0 }, 2, 0,ang);
		//	particles.push_back(Particle(pos.x, pos.y, pos.z, j));
		//	particles[j].setDestination({ 0, 1.5,0.1 });
		//	particles[j].setSleep(1);
		//}
		//for (j; j < 200; j++) {
		//	float ang = 1 * (j + 1);
		//	vec3 pos = spawnRandomCircle({ 0,1.5,0 }, 2, 0,ang);
		//	particles.push_back(Particle(pos.x, pos.y, pos.z, j));
		//	particles[j].setDestination({ 0, 0,0.1 });
		//	particles[j].setSleep(1);
		//}
		...
		//createSetUpFile(i);
		  ...
		  break;
	case 1: {
		simulationName = "horizontalBridge";

		platforms.push_back(Platform(-1.0, 1.0, 0.0, box, .9, .1, .1, 0));
		platforms.push_back(Platform(-2.0, 1.0, 0.0, box, .2, 2, 2, 1));
		platforms.push_back(Platform(1.0, 1.0, 0.0, box, .9, .1, .1, 2));
		...
		for (j; j < 50; j++) {
			float ang = 1 * (j + 1);
			vec3 pos = spawnRandomCircle({ -.5,1,0 }, 1, 2, ang);
			particles.push_back(Particle(pos.x, pos.y, pos.z, j));
			int rem = j % 10;
			if (rem != 0) {
				particles[j].follow = true;
				particles[j].followId = j - rem;
				particles[j].speed = .025;
				particles[j].state = FollowLeader;
				//particles[j].setDestination({ 0, 1.5,0.1 });
			}
		...
		//for (j; j < 150; j++) {
		//	vec3 pos = spawnRandomCircle({ -1,1,0 }, 2, 2, ang);
		//	particles.push_back(Particle(pos.x, pos.y, pos.z, j));
		//	particles[j].setDestination({ 3, 2,0 });
		//	particles[j].setSleep(1);
		//}
		...
		//createSetUpFile(i);
		  ...
		  break;
	case 2: {
		simulationName = "testRoom";

		//platforms.push_back(Platform(0.0, 0.0, 0.0, box, .2, .9, .2, 0));
		platforms.push_back(Platform(0.0, -0.3, 0.0, box, 2, .2, 2, 0));
		//platforms.push_back(Platform(0.0, 2.0, 0.0, box, .2, .9, .2, 2));
		platforms.push_back(Platform(0.0, 2.0, 0.0, box, 2, .2, 2, 1));

		setCameraPosition(0, 1, 5);

		particles.push_back(Particle(-5, 0, 0, 0));
		particles.push_back(Particle(5, 0, 0, 1));
		particles[0].destination = { 1,0,0 };
		particles[1].destination = { -1,0,0 };
		particles[0].state = FollowDestination;
		particles[1].state = FollowDestination;
	}
		  break;
	}
