...
const float mui_static = 0.00021f;    // 0.00023; //0.021;
const float mui_kinematic = 0.00012f; // 0.00017; //0.02;
		...
		if (startObjectTrace == false) {
			...
			if (obsticleCollisionId != -1) {
				if (collWith == 'p') {
					...
					dir = Direction::left;// getRandomDir();
				}
				else if (collWith == 'a') {
...
float* friction_constraint(Particle p1, Particle p2) {
	...
	float friction_constraint_stiffness = .0002;
...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < 100; i++) {
		...
		particles[i].setDestination({ 1.0, .10, 1.0 });
		x = ((float)rand()*1 / (RAND_MAX)) + 1;
		z =((float)rand()*1 / (RAND_MAX)) + 1;
	}
	...
	platforms.push_back(Platform(-2.9, .50, -2.0, sphere, .3, 3));
