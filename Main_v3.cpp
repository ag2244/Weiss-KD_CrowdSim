...
public:
	...
	void update() {
		...
		if (timer == 0) {
			...
			if (isStatic == false) {
				if (distance(x, y, z, timerPosition.x, timerPosition.y, timerPosition.z) < r * 1.5) {
					isStatic = true;
				}
...
float* axis_aligned_cube_collision_constraint(Particle p1,Platform cube) {
	...
	float x_grad = p1.px - cube.x;
	float y_grad = p1.py - cube.y;
	float z_grad = p1.py - cube.z;
	float dx = abs(x_grad) - cube.w / 2 - p1.r;
	float dy = abs(y_grad) - cube.h / 2 - p1.r;
	float dz = abs(z_grad) - cube.l / 2 - p1.r;
	float delta_x1 = 0.0;
	float delta_y1 = 0.0;
	float delta_z1 = 0.0;
	if (dx < 0 && dy < 0 && dz < 0){
		...
		if (dx < dy) {
			if (dx < dz)
				delta_x1 = x_grad / abs(x_grad) * abs(dx);
			else
				delta_z1 = z_grad / abs(z_grad) * abs(dz);

		}
		else {
			if (dy < dz)
				delta_y1 = y_grad / abs(y_grad) * abs(dy);
			else
				delta_z1 = z_grad / abs(z_grad) * abs(dz);
		}
	}
...
float* collision_constraint(Particle particle1, Particle particle2) {
	...
	if (dist_diff < 0) {
		...
		correction_y1 = coef1 * coef * (pz1 - pz2);
		correction_x2 = coef2 * coef * (px1 - px2);
		correction_y2 = coef2 * coef * (py1 - py2);
		correction_y2 = coef2 * coef * (pz1 - pz2);
	}
...
void pbd_main_loop(int a)
{
	...
	glutPostRedisplay();
	glutTimerFunc(2, pbd_main_loop, 2);//Call update after each 25 millisecond
}
...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < 100; i++) {
		...
		particles[i].setDestination({ 1.0, 5.0, 1.0 });
	...
	platforms.push_back(Platform(0.0, 0.0, 1.0, box, .2, 3, .2, 5));
